...
function shoreditch_jetpack_scripts() {
	if ( is_home() && ! is_paged() && ! empty( shoreditch_get_featured_posts() ) ) {
		wp_register_script( 'shoreditch-flexslider', get_template_directory_uri() . '/js/jquery.flexslider.js', array( 'jquery' ), '2.6.0', true );

		wp_enqueue_script( 'shoreditch-featured-content', get_template_directory_uri() . '/js/featured-content.js', array( 'shoreditch-flexslider' ), '20160411', true );
	}
}
